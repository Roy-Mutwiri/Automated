...
`tools/autostart.ps1` registers one scheduled task per worktree,
`AutomatedRepoSync_<worktree>`, each launching `sync.ps1 -Worktree <its own
directory>`. A watcher can therefore only ever commit and push the branch it is
sitting on. The legacy single `AutomatedRepoSync` task, which watched the shared
repo and attributed by folder, is unregistered.

Registered is not the same as running. Current state:

| Task | Worktree | Enabled |
|---|---|---|
| `AutomatedRepoSync_Automated` | `Automated` (main) | yes |
| `AutomatedRepoSync_Automated-camera` | `Automated-camera` | **no** |
| `AutomatedRepoSync_Automated-movement` | `Automated-movement` | yes |

The camera watcher is deliberately disabled: camera commits are written
deliberately, not swept up on a debounce. Enable it if that changes:

    Enable-ScheduledTask -TaskName AutomatedRepoSync_Automated-camera
    Start-ScheduledTask  -TaskName AutomatedRepoSync_Automated-camera

Or run one in the foreground for a single session, without registering anything:

    powershell -ExecutionPolicy Bypass -File tools\sync.ps1 `
        -Worktree C:\Users\mutwi\Documents\Automated-camera

Stopping your own watcher — `Stop-ScheduledTask -TaskName
AutomatedRepoSync_<worktree>` — is now enough to rewrite your own history, and
affects no other terminal.
